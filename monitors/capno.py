...
class Capnograph:
    ...
    def build_context(resp_state, vent_rr: float, insp_fraction: float, vent_active: bool) -> CapnoContext:
        ...
        if vent_active and vent_rr > 0.1:
            ...
            # Clinical criteria for visible cleft:
            # 1. Significant central drive (emerging from anesthesia, drive > 0.3)
            # 2. Adequate muscle function (not paralyzed, muscle > 0.2)
            # 3. Either meaningful effort signal OR spontaneous RR notably > vent RR
            #
            # Very weak efforts (e.g., spont RR=1, drive=0.1) should NOT cause cleft
            curare_active = (
                drive > 0.3 and           # Meaningful central drive (emerging)
                0.1 < muscle < 0.9 and    # Partial block ONLY (not paralyzed, not full strength)
                (effort_signal > 0.15 or  # Significant effort (drive*muscle)
                 delta_rr > 2.0)          # Spontaneous RR notably exceeds vent
            )
        ...
        if phase == "INSP":
            ...
            if self.time_in_phase < deadspace_time:
                 ...
                 if curare_cleft and not is_spontaneous:
                     ...
                     # Only render if we are past the cleft point
                     # Dip parameters
