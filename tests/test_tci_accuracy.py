...
These tests verify that the TCI controller correctly achieves and maintains
target concentrations for propofol and remifentanil.

Validation criteria:
- Target tracking: Ce should reach within 10% of target within 60 seconds
- Steady-state accuracy: Ce should be within 5% of target at steady state
- No overshoot > 20% during induction
"""

import pytest
import numpy as np
from patient.patient import Patient
from patient.pk_models import PropofolPKSchnider, RemifentanilPKMinto
...
class TestTCIPropofol:
    ...
    def test_plasma_target_mode(self):
        """Test TCI in plasma-targeting mode."""
        patient = Patient(age=50, weight=75, height=175, sex="Male")
        pk = PropofolPKSchnider(patient)
        tci = TCIController(
            pk_model=pk,
            drug_name="Propofol",
...
class TestTCIInitialization:
    ...
    def test_set_state_priming(self):
        ...
        # Controller should recognize we're at target and give low rate
        rate = tci.step(target)
        
        # Rate should be relatively low (maintenance, not induction)
