...
from patient.pk_models import (
    PropofolPKMarsh, PropofolPKSchnider, 
    RemifentanilPKMinto, NorepinephrinePK, PhenylephrinePK, RocuroniumPK,
    EpinephrinePK
)
from patient.pd_models import TOFModel
...
class TestPKModelInit:
    ...
    def test_propofol_marsh_init(self, patient):
        model = PropofolPKMarsh(patient)
        assert model.v1 > 0
        assert model.k10 > 0
        A, B = model.get_ss_matrices()
        assert A.shape == (4, 4)
        assert B.shape == (4, 1)

    def test_propofol_schnider_init(self, patient):
        model = PropofolPKSchnider(patient)
        assert model.v1 > 0
        
    def test_remi_minto_init(self, patient):
        model = RemifentanilPKMinto(patient)
        assert model.v1 > 0
        A, B = model.get_ss_matrices()
        assert A.shape == (4, 4)

    def test_pk_step_mechanics(self, patient):
        model = PropofolPKMarsh(patient)
        dt = 1.0
        infusion = 10.0 # mg/s
        
        state_0 = model.state.c1
        model.step(dt, infusion)
        state_1 = model.state.c1
        
        assert state_1 > state_0

    def test_vasopressors_init(self, patient):
        epi = EpinephrinePK(patient)
        assert epi.v1 > 0
        
        phenyl = PhenylephrinePK(patient)
        assert phenyl.v1 > 0
        
        norepi = NorepinephrinePK(patient, model="Beloeil")
        assert norepi.v1 > 0


# --- TCI Controller Tests ---

class TestTCI:
    def test_propofol_tci(self, patient):
        pk = PropofolPKMarsh(patient)
        tci = TCIController(pk, 'Propofol', 'effect_site')
        
        # Test basic targeting
        target = 2.0 # ug/mL
        rate = tci.step(target)
        assert rate > 0 # Should bolus
        
        # Zero target
        rate_zero = tci.step(0.0)
        assert rate_zero == 0.0

    def test_vasopressor_tci_integration(self, patient):
        """
        Verify that TCI controllers work specifically for vasopressors,
        which was a previous bug source.
        """
        # Epinephrine
        epi_pk = EpinephrinePK(patient)
        epi_tci = TCIController(epi_pk, 'Epinephrine')
        epi_rate = epi_tci.step(1.0) # Target 1 ng/mL
        assert epi_rate >= 0, "Epi TCI should return valid rate"
        
        # Phenylephrine
        phenyl_pk = PhenylephrinePK(patient)
        phenyl_tci = TCIController(phenyl_pk, 'Phenylephrine')
        phenyl_rate = phenyl_tci.step(10.0) # Target 10 ng/mL
        assert phenyl_rate >= 0, "Phenyl TCI should return valid rate"
...
class TestPharmacologySanity:
    ...
    and expected physiological behaviors (validity checks).
    
    References:
    - Propofol: Marsh 1991, Schnider 1998, Robinson 1997
    - Remifentanil: Minto 1997, Egan 1993
    - Neuromuscular: Wierda 1991 (Rocuronium)
    - Vasopressors: Beloeil 2005 (Norepi), various standard PK texts.
    """
    # --- Propofol ---
...
class TestVasopressorPK:
    ...
    def test_norepinephrine_beloeil_kinetics(self, patient):
        """
        Beloeil 2005 model: 1-compartment with SAPS-II adjusted clearance.
    ...
    def test_norepinephrine_li_propofol_interaction(self, patient):
        """
        Li 2024 model: Propofol concentration affects norepinephrine clearance.
    ...
    def test_vasopressor_effect_site_delay(self, patient):
        ...
        equilibrates more slowly - we allow 3 minutes for equilibration.
        """
...
class TestNeuromuscularRecovery:
    ...
    - Pühringer FK et al. Br J Anaesth 2010
    ...
    def test_sugammadex_moderate_block(self, patient):
        ...
        Nguyen-Lee 2018, de Kam 2011: typical time 1.3-2.0 min.
        """
        pk = RocuroniumPK(patient, model_name="Wierda")
        pd = TOFModel(patient, model_name="Wierda")
    ...
    def test_sugammadex_immediate_reversal(self, patient):
        ...
        Ploeger/de Kam population PK-PD analysis.
        """
        pk = RocuroniumPK(patient, model_name="Wierda")
        pd = TOFModel(patient, model_name="Wierda")
