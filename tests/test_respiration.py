import pytest
import math
from physiology.respiration import RespiratoryModel, RespState
...
class TestDrugParameters:
    def test_hill_propofol_separated(self, model):
        # Sarton 2001 / JPET 2024: Separated propofol effects
        # HCVR: C50 = 1.0, Gamma = 2.0 (early potent effect on CO2 sensitivity)
        # Mechanical: C50 = 3.5, Gamma = 2.0 (higher doses for VT/RR depression)
        
        # Checking internal parameters
        assert model.c50_prop_hcvr == 1.0
        assert model.gamma_prop_hcvr == 2.0
        assert model.c50_prop_mech == 3.5
        assert model.gamma_prop_mech == 2.0
        
    def test_hill_remi(self, model):
        # C50 = 1.0, Gamma = 1.25
        assert model.c50_remi == 1.0
        assert model.gamma_remi == 1.25
        # Setpoint shift added
        assert model.remi_setpoint_shift_max == 8.0

    def test_hill_sevo_mac(self, model):
        # C50 = 0.75 MAC (adjusted for human data)
        assert model.c50_sevo_mac == 0.75
        assert model.gamma_sevo == 2.0

class TestCentralDriveVsMuscle:
    def test_nmba_spares_central_drive(self, model):
        """NMBA preserves central drive but zeroes observed RR (no muscle function).
...
class TestDifferentialScaling:
    def test_remi_rr_vs_vt(self, model):
        """Remifentanil should depress RR more than VT."""
        dt = 1.0
        # Remi C50 = 1.0. 
        # w_remi_rr = 1.0, w_remi_vt = 0.5
        
        # At C50 (1.0 ng/mL), eff_remi = 0.5.
        # rr_inhib = 1.0 * 0.5 = 0.5 -> RR frac = 0.5
        # vt_inhib = 0.5 * 0.5 = 0.25 -> VT frac = 0.75
        
        s = model.step(dt, ce_prop=0, ce_remi=1.0, ce_roc=0)
        
        rr_frac = s.rr / model.rr_0
        vt_frac = s.vt / model.vt_0
        
        assert rr_frac == pytest.approx(0.5, abs=0.05)
        assert vt_frac == pytest.approx(0.75, abs=0.05)
        assert rr_frac < vt_frac # RR more affected

    def test_prop_vt_vs_rr(self, model):
        """Propofol should depress VT more than RR."""
        dt = 1.0
        # Separated propofol effects: Mechanical C50 = 3.5 µg/mL
        # w_prop_rr = 0.6, w_prop_vt = 1.0
        
        # At C50 for mechanical effect (3.5 ug/mL), eff_prop_mech = 0.5
        # rr_inhib = 0.6 * 0.5 = 0.3 -> RR frac = 0.7
        # vt_inhib = 1.0 * 0.5 = 0.5 -> VT frac = 0.5
        
        s = model.step(dt, ce_prop=3.5, ce_remi=0, ce_roc=0)
        
        rr_frac = s.rr / model.rr_0
        vt_frac = s.vt / model.vt_0
        
        assert rr_frac == pytest.approx(0.7, abs=0.05)
        assert vt_frac == pytest.approx(0.5, abs=0.05)
        assert vt_frac < rr_frac  # VT more affected than RR
...
class TestRespiratoryLiteratureValidation:
    ...
    References:
    - Opioid respiratory depression: Egan TD, Anesthesiology 1993; Dahan A, BJA 2010
    - Propofol: Goodman LS, Pharmacology 12th ed; Shafer SL, Anesthesiology
    - NMBA: Wierda, Masui - neuromuscular block physiology
    """
    ...
    def test_baseline_tidal_volume(self, model):
        """
        LITERATURE: Normal adult tidal volume is 6-8 mL/kg (~400-600 mL for 70kg).
        """
        state = model.step(1.0, ce_prop=0, ce_remi=0, ce_roc=0)
        
        assert 300 < state.vt < 800, \
            f"Baseline VT {state.vt:.0f} mL outside physiological range (300-800)"
    
    def test_baseline_minute_ventilation(self, model):
        """
        LITERATURE: Normal minute ventilation is ~5-8 L/min at rest.
        """
        state = model.step(1.0, ce_prop=0, ce_remi=0, ce_roc=0)
        
        mv_l_min = (state.rr * state.vt) / 1000.0
        
        assert 3.0 < mv_l_min < 12.0, \
            f"Baseline MV {mv_l_min:.1f} L/min outside physiological range (3-12)"
    ...
    # --- NMBA Effects (Central Drive vs Muscle) ---
    
    def test_nmba_preserves_central_drive(self, model):
        """
        LITERATURE: NMBAs block neuromuscular junction, NOT central respiratory drive.
        High-dose NMBA should reduce VT (muscle paralysis) but NOT reduce central drive.
        """
        # High-dose rocuronium
        state = model.step(1.0, ce_prop=0, ce_remi=0, ce_roc=10.0)
        
        # Central drive should be preserved (near 1.0)
        assert state.drive_central > 0.9, \
            f"With NMBA, central drive {state.drive_central:.2f} should be >0.9 (preserved)"
        
        # Muscle factor should be near 0 (paralysis)
        assert state.muscle_factor < 0.15, \
            f"With NMBA, muscle factor {state.muscle_factor:.2f} should be <0.15 (paralysis)"
    
    def test_nmba_vt_near_zero(self, model):
        """
        LITERATURE: Full neuromuscular block eliminates VT (patient cannot breathe).
        """
        state = model.step(1.0, ce_prop=0, ce_remi=0, ce_roc=10.0)
        
        # VT should be near zero
        assert state.vt < 50, \
            f"With full NMBA block, VT {state.vt:.0f} mL should be near zero"
    ...
    def test_sevoflurane_respiratory_depression(self, model):
        """
        LITERATURE: Volatile anesthetics cause dose-dependent respiratory depression.
        At 1 MAC, expect moderate RR/VT depression.
        """
        # Note: Current model uses mac_sevo parameter in engine, not directly in RespiratoryModel
        # This test verifies model parameters exist
        
        assert hasattr(model, 'c50_sevo_mac'), "Model should have volatile PD parameters"
        assert model.c50_sevo_mac > 0, "Volatile C50 should be positive"
...
class TestPaO2Physiology:
    ...
    They reduce CaO2 (oxygen content) and DO2 (delivery), but PaO2
...
class TestHypercapnicVentilatoryResponse:
    ...
    - Bouillon 2003: Opioids depress HCVR slope by 50-70% AND shift setpoint right
    - Dahan 1996: Volatiles depress HCVR by ~50% at 1 MAC
    """
    
    def test_hcvr_parameters_exist(self, model):
        """HCVR parameters should be initialized."""
        assert hasattr(model, 'hcvr_slope_baseline')
        assert hasattr(model, 'paco2_setpoint')
        assert hasattr(model, 'hcvr_depression_remi')
        assert hasattr(model, 'hcvr_depression_prop')
        assert hasattr(model, 'hcvr_depression_sevo')
        assert hasattr(model, 'remi_setpoint_shift_max')  # New setpoint shift
        assert model.hcvr_slope_baseline > 0
        assert model.paco2_setpoint == 40.0
        assert model.remi_setpoint_shift_max == 8.0
    ...
    def test_opioids_depress_hcvr(self, patient):
        """
        LITERATURE: Opioids depress HCVR slope by 50-70%.
    ...
    def test_remifentanil_setpoint_shift(self, patient):
        ...
        Bouillon 2003: Opioids shift CO2 curve right AND reduce slope.
        """
