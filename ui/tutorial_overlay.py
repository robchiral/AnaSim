...
class ScenarioOverlay(QFrame):
    ...
    def _update_progress(self):
        ...
        # Update progress bar
        # If we are done (current_step == total), show 100%
...
def TutorialOverlay(mode="awake", maint_type="tiva", parent=None):
    ...
    """
    from .scenarios.anaphylaxis import create_anaphylaxis_scenario
    
    # Simple heuristic mapping; expand if config passes specific IDs.
    if mode == "anaphylaxis_response":
        scenario = create_anaphylaxis_scenario()
    elif mode == "hemorrhage_response":
