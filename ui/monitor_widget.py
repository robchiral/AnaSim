...
class PatientMonitorWidget(QWidget):
    ...
    def update_patient_info(self, name="Simulated patient", age=40, gender="M", weight=70):
        ...
        self.lbl_patient.setText(f"Patient: {name}  •  {age}y {gender}  •  {weight}kg")
    ...
    def update_waveforms(self, engine):
        ...
        # Find new data points since last update
        # We can just iterate backwards until we find time <= last_plot_time
        # (This implies strictly increasing time, which is true for sim)
        ...
        self.last_plot_time = new_states[-1].time
